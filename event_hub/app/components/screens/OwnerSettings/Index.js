...
export default class Index extends Component {
  ...
  // this will decide which type of company we are going to show
  CheckType() {
  ...
  OwnerDashboardDetail() {
    ...
    if (this.state.typeOfCompany == "Hall") {
      url =
        "https://eventhub-api.conveyor.cloud/api/Owner/OwnerDashboard?id=" +
        global.id;
    } else if (this.state.typeOfCompany === "Media") {
      url =
        "https://eventhub-api.conveyor.cloud/api/Owner/OwnerDashboardMedia?id=" +
        global.id;
    } else if (this.state.typeOfCompany === "Caterer") {
      url =
        "https://eventhub-api.conveyor.cloud/api/Owner/OwnerDashboardCaterer?id=" +
        global.id;
    }
  ...
  componentDidMount() {
    ...
    this.OwnerDashboardDetail();
  }
  decideNavigation(nav) {
    const type = this.state.typeOfCompany;
    var obj = this.state;
    nav.navigate("Owner Completed", { Object: obj });
  }
  decideNavigation1(nav) {
    const type = this.state.typeOfCompany;
    var obj = this.state;
    nav.navigate("Owner Continue", { Object: obj });
  }
  decideNavigation2(nav) {
    const type = this.state.typeOfCompany;
    var obj = this.state;
    nav.navigate("Owner Pending", { Object: obj });
  }
  render() {
          ...
          <GenericPicker
            ...
            On_Value_Change={(value, index) =>
              this.setState({ typeOfCompany: value }, () => {
                ...
                this.OwnerDashboardDetail.bind(this)(nav);
                global.typeOfCompany = this.state.typeOfCompany;
              })
            }
          />
        ...
        <View style={styles.Total}>
          ...
          >
            <View>
              ...
              <Text style={styles.t2}>TOTAL REQUESTS</Text>
              <Text style={styles.t2}>RECEIVED</Text>
            </View>
        ...
        <View style={{ flex: 1, flexDirection: "row", marginTop: hp("1%") }}>
          <View style={styles.Progress}>
            <View>
              ...
              <Text style={styles.t2}>REQUESTS</Text>
            </View>
            <View style={styles.i2}>
              <Icon name={"timelapse"} size={60} color="#FFFFFF" />
            </View>
          </View>
          <View style={styles.completed}>
            ...
            <View style={styles.i2}>
              <Icon name={"offline-pin"} size={60} color="#FFFFFF" />
            </View>
          </View>
        </View>
