...
class CompletedBooking extends Component {
  constructor(props) {
    super(props);
    this.state = {
      typeOfBooking: "CompletedCustomer",
      data: ""
    };
  }
  CompletedBookingForHall() {
    var url;
    if (global.typeOfCompany === "Hall") {
      url =
        "https://eventhub-api.conveyor.cloud/api/User/CustomerCompletedRequestsHall?id=" +
        global.id;
    } else if (global.typeOfCompany === "Media") {
      url =
        "https://eventhub-api.conveyor.cloud/api/Media/CustomerCompletedRequestsMedia?id=" +
        global.id;
    } else if (global.typeOfCompany === "Caterer") {
      url =
        "https://eventhub-api.conveyor.cloud/api/Caterer/CustomerCompletedRequestsCaterer?id=" +
        global.id;
    }
    fetch(url, {
      method: "POST",
